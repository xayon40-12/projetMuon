...
void PlotHisto(char* filename){
  ...
  Double_t bestdebut=0;
  int bestbinning=0;
  
  
  for (int binning=90;binning<120;binning++){
for (Double_t debut=0;debut<1;debut+=0.07){
  h2 = new TH1F("h2","Donnees simulation",binning,debut,10.) ;
  h2->Sumw2();
  ...
  TFitResultPtr MyFit = h2->Fit(fit,"LS","E",0.35,9.8);
  ...
  c1->BuildLegend();
  cout << "bestchi2 " << bestchi2 << "\nbest binning " << bestbinning << "\nbestdebut " << bestdebut << endl;
